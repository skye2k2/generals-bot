...
const ai = {
	...
	/**
	 * Recursive! For each space, if there is an immediately-available army, add an attack to the queue.
	 ...
	 * @returns {undefined} - Pushes any discovered moves to the attack queue directly.
	 */
	generateAllSimpleAttacks: function (spacesToAttack, availableArmies) {
		...
		const targetIndex = spacesToAttack.shift()
		let attackerIndex = -1
		let indexInArray

		// See if any free armies of sufficient strength neighbor the target space
		for (let idx = 0; idx < availableArmies.length; idx++) {
			const spaceInfo = this.calculateTileLocationInfo(availableArmies[idx].locationIndex)
	 ...
	 * @param {string} mode - The mode to associate with the moves being added.
	 * @param {Array} moveList - The list of moves to process.
	 */
	...
	/**
	 * Recursive! Find a path from the start to the end location.
	 * @param {number} currentLocationIndex - From here.
	 * @param {number} destinationLocationIndex - To here.
	 * @param {Array} pathToTarget - The accumulated path we have so far between the start and the end location.
	 * @param {Array} badTiles - List of location indices to avoid.
	 * @returns {Array} - An array of location indices that lead from the start to the destination.
	 */
	...
	/**
	 * For a given location, calculate the details of the surrounding tiles.
	 * @param {number} indexToCheck - The index to caclculate the surrounding tile information for.
	 * @returns {object} - An object with the tile's own information, as well as that for each of the cardinal directions.
	 */
	 ...
	 * @param primaryDirectionToFlood
	 * @param secondaryDirectionToFlood
	 * @param availableArmies
	 */
	...
	// 		const chosenDirection = DIRECTIONS_MAP[Math.floor(Math.random() * 4)] // Randomly pick a number between 0 and 3
